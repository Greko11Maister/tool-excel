...
func Compare_Files(X[][] string, Y[][] string, indexColF1 int, indexColF2 int){
	...
	for i := 0; i < len(X); i++{
		...
		if (!found) {fmt.Println("No se encontro File 1 row: (",i,") ",X[i][indexColF1])}

	}

}

func IndicesFilesForSearchs(X[][] string)(a string){
	var Options string
	for i:= 0; i < len(X); i++  {
		for e:= 0; e < len(X[i]); e++  {
			if(i==0){Options = Options+" | ("+strconv.Itoa(e)+") "+X[i][e];}
		}
	}
